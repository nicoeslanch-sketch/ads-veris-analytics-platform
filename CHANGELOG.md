...
## [0.23.0] - 2026-07-22 - Fase 19: control de calidad PYME 2026, Resumen numérico y Explorar que decide

Auditoría propia contra `Prueba_PYME_Desafiante_Multihoja_ADS_VerIs_2026`
(reproduje cada hallazgo del control de calidad externo con el motor real) y
reestructura editorial: **Resumen muestra números y resultados contables;
Explorar datos interpreta y ayuda a decidir** (`docs/ANALISIS_FINANCIERO.md`).

### Motor y limpieza (hallazgos corregidos, verificados con el archivo real)

- Filas `TOTAL 2024/2025/2026`: detectadas como totales estructurales, se
  conservan en la base y se EXCLUYEN de todos los indicadores — sumarlas
  duplicaba los ingresos del periodo (los KPI combinados pasaban de ~$27,6 mil
  millones a los ~$13,75 mil millones reales). Señaladas en Observaciones.
- `Total Documento` volvía como TEXTO: el token "documento" activaba la
  heurística de identificador. Los marcadores monetarios (total, monto, neto,
  IVA…) ahora vencen a los tokens débiles de documento; `ID Pago` y
  `Codigo Documento` siguen siendo identificadores. 5.070/5.070 celdas
  numéricas verificadas en el export.
- Validaciones aritméticas por fila en Observaciones: IVA ≈ tasa INFERIDA de
  la propia columna × neto, y Total ≈ neto + IVA — descuadres señalados,
  jamás corregidos en silencio.
- Costos peligrosos celda a celda: negativos, en cero y atípicos (IQR).
- Duplicados POSNORMALIZACIÓN listados fila a fila (37/37 del archivo de
  prueba) como tipo propio en Observaciones.
- Columnas de texto libre (Observación, Comentario, Nota…) ya no generan
  alertas de "faltante": 3.353 avisos de ruido eliminados (4.246 → 1.758
  observaciones, todas accionables).
- Manifest honesto: el alcance analítico vive en la fila "(libro completo)"
  de la hoja Manifest; cada hoja solo lo declara si participa. El
  `manifest.json` del ZIP CSV conserva su contrato.
- Exportación más rápida: los parseos numéricos de las validaciones se
  calculan una vez por columna y se reutilizan (44,8s → 31,2s en frío para el
  libro de 16 hojas en el equipo de prueba; el segundo clic sigue siendo
  instantáneo por la caché de bytes).

### Resumen (números) vs Explorar (decisiones)

- Explorar gana "Rentabilidad para decidir": mapa del portafolio
  (participación × margen contra las MEDIANAS del propio archivo) con los
  cuadrantes Estrella / Vaca lechera / Oportunidad / Problema y su acción
  estratégica; productos que destruyen margen; ventas bajo el costo con la
  pérdida bruta que explican; márgenes atípicos por fila; margen bruto mes a
  mes. Todo con gráficos e interpretación, no solo cifras.
- El "Detalle mensual" de Explorar siempre muestra Costo, Utilidad y Margen
  cuando hay costos (la vista ventas+costos ya no cuenta solo ingresos).
- Resumen suma ratios de riesgo del analista: concentración del producto top
  (con alerta sobre 25%) y tasa de anulación.
- Nuevo `analisis_rentabilidad` en /metrics (bloqueado con moneda mixta).

### UX

- El panel "Duplicados exactos" explica el ALCANCE del botón: elimina los de
  la hoja seleccionada, o bien limpiar primero y usar "Eliminar duplicados de
  todas" para hacerlo global.

### Documentación

- `docs/ANALISIS_FINANCIERO.md`: guía completa de ratios (liquidez, CCC,
  endeudamiento, DuPont, LTV/CAC, GMROI, punto de equilibrio), modelo
  relacional del negocio, uniones seguras (as-of join de costos históricos,
  agregación de cobranzas), red flags y mapa implementado vs pendiente.

Sube `ENGINE_VERSION` a `0.23.0`. Sin migraciones nuevas (0021 es la última).
