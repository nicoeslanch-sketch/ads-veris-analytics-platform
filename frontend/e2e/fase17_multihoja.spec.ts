...
test('Fase 17 procesa, combina, relaciona y exporta un libro multihoja', async ({ page }, testInfo) => {
  ...
  await page.goto('/estandarizacion')
    ...
    await analysis.getByTestId('exploration-methodology').locator('summary').click()
    ...
    await expect(analysis.locator('.recharts-wrapper').first()).toBeVisible()
})
...
test('perfil de cobranza nominal adapta KPI, filtros y detalle sin inventar transacciones', async ({ page }, testInfo) => {
  ...
  for (const dimension of ['Forma de pago', 'Período cotizado', 'Descripción del pago']) {
    ...
    await expect(analysis.getByRole('heading', { name: `Recaudación de cobranza por ${dimension.toLowerCase()}` })).toBeVisible()
