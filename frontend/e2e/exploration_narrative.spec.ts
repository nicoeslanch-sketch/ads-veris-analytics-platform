...
test('Explorar explica evidencia, conserva controles y no desborda en movil', async ({ page }, testInfo) => {
  ...
  await expect(analysis.getByRole('heading', { name: 'Oportunidades para revisar' })).toBeVisible()
  await expect(analysis.getByRole('heading', { name: 'Qué limita las conclusiones' })).toBeHidden()
  await expect(analysis.locator('.recharts-wrapper')).toHaveCount(2)
  await analysis.getByRole('button', { name: 'Menor valor', exact: true }).click()
  await expect(analysis.getByRole('button', { name: 'Menor valor', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await analysis.getByRole('button', { name: 'Mayor valor', exact: true }).click()
  ...
  await expect(analysis.getByRole('heading', { name: 'Ingresos por producto', exact: true })).toBeVisible()
  ...
  await analysis.getByTestId('exploration-methodology').locator('summary').click()
  await page.screenshot({ path: testInfo.outputPath('exploration-compact-desktop.png'), fullPage: true })
