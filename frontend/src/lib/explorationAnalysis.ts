...
export function explorationActions(measure: ExplorationMeasure, metrics: MetricsResult, dimension?: string, from?: string | null, to?: string | null): Array<{ title: string; evidence: string; action: string }> {
  ...
  if (leader) {
    ...
    actions.push({ title: expense ? 'Revisar costos o gastos destacados' : profit ? 'Comparar rentabilidad del grupo' : measure.id === 'ingresos' ? 'Priorizar una revisión comercial' : 'Comparar el grupo destacado', evidence: leader.name + ' registra ' + fmt(leader.value) + ' en ' + (selected?.name ?? 'el desglose') + '; es el mayor valor entre los grupos disponibles.', action: expense ? 'Contrasta volumen, contratos y necesidad del gasto antes de recortar; un importe alto no demuestra ineficiencia.' : profit ? 'Contrasta margen, volumen y cobertura de costos; una utilidad mayor no implica mayor margen ni caja disponible.' : measure.id === 'ingresos' ? 'Comprueba margen, disponibilidad y recurrencia antes de reforzar compras o promoción; más ingresos no garantizan más utilidad.' : 'Revisa el tamaño y la cobertura del grupo antes de atribuirle mejor desempeño.' })
