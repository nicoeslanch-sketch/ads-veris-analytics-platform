...
ARTICLES: list[dict[str, Any]] = [
    ...
    article("explore", "analisis", "Qué muestra Explorar datos", ["explorar datos", "diferencia resumen", "detalle", "profundizar"], "Explorar combina evolución y comparación de grupos con hasta tres sugerencias breves. Puedes cambiar medida y desglose; la calidad, las conexiones por ID y la evidencia numérica quedan desplegables. Ayuda a decidir qué investigar, sin presentar una diferencia como una causa demostrada."),
    article("storage_quota", "datos", "Cuota de almacenamiento", ["cuota de almacenamiento", "limite de almacenamiento", "almacenamiento lleno", "espacio lleno", "storage quota", "error 507", "espacio disponible", "cuanto espacio tengo", "cuantos archivos puedo guardar", "cuantos archivos puedo subir"], "Consulta el espacio usado en Configuración, Almacenamiento. La cuota cuenta originales y artefactos guardados; las subidas pendientes también reservan espacio. Si se alcanza, se rechazan nuevas escrituras sin borrar tus archivos existentes. Libera espacio desde Historial. La cuota no cambia las reglas de limpieza ni elimina duplicados.", "¿El aviso aparece al subir o al descargar?", 88),
...
def answer_for(
    ...
    *,
    ...
    if 'resumen' in normalized and 'explorar' in normalized:
        return {"answer": "Resumen presenta los KPIs del alcance visible. Explorar combina graficos de evolucion y grupos con una lectura explicativa breve: hasta tres sugerencias para investigar. La evidencia numerica, cobertura de conexiones por ID y limites quedan desplegables. Una diferencia observada no demuestra por si sola una causa.", "matched_key": "explore", "confidence": "high", "suggestions": ["Filtros activos", "Calidad de los datos"]}
